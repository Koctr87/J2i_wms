...
if (isset($_GET['lang']) && in_array($_GET['lang'], SUPPORTED_LANGUAGES)) {
    ...
    if (!empty($_SESSION['user_id'])) {
        try {
            ...
            // Silently fail - language will still work via session
        }
    }

    // Redirect to same page without ?lang= to prevent "headers already sent" on pages
    $redirectUrl = strtok($_SERVER['REQUEST_URI'], '?');
    // Preserve other query params except 'lang'
    $queryParams = $_GET;
