...
$availableDevices = $db->query("
    SELECT d.id, b.name as brand_name, p.name as product_name
    FROM devices d
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['accessory_id']) && !empty($_POST['accessory_id'])) {
            // Edit existing
            $accId = (int) $_POST['accessory_id'];
            
            // Get old quantity to adjust quantity_available
            $oldAcc = $db->query("SELECT quantity, quantity_available FROM accessories WHERE id = $accId")->fetch();
            $quantity_diff = $quantity - $oldAcc['quantity'];
            $new_available = $oldAcc['quantity_available'] + $quantity_diff;
            
            $stmt = $db->prepare("
                UPDATE accessories SET 
                type_id = ?, name = ?, purchase_date = ?, quantity = ?, quantity_available = ?,
                    ...
                    <tbody>
                        <?php foreach ($accessories as $acc): ?>
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-outline" onclick="openEditModal(<?= e(json_encode($acc)) ?>)">
            ...
            <?php if ($pagination['total_pages'] > 1): ?>
                <div class="card-footer" style="display: flex; justify-content: center; gap: 0.5rem; padding: 1rem;">
                    <?php if ($pagination['has_prev']): ?>
                        <a href="?page=<?= $page - 1 ?>" class="btn btn-sm btn-secondary">←</a>
                    <?php endif; ?>
...
<div class="modal-overlay" id="accessoryModal">
    <div class="modal" style="max-width: 600px;">
        ...
        <form method="POST" action="" id="accessoryForm">
            <input type="hidden" name="accessory_id" id="accId">
            <div class="modal-body">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="purchase_date" id="purchaseDate" class="form-control" value="<?= date('Y-m-d') ?>"
                            required>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="accName" class="form-control"
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" name="quantity" id="accQuantity" class="form-control" value="1" min="1" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 0.5rem;">
                            <input type="number" name="purchase_price" id="purchasePrice" class="form-control" step="0.01" min="0" required
                                style="flex: 1;">
                            <select name="purchase_currency" id="purchaseCurrency" class="form-control" style="width: 80px;">
                ...
                <div class="form-group" id="repairCommentGroup" style="display: none;">
                    ...
                    <textarea name="repair_comment" id="repairComment" class="form-control" rows="2"
                ...
                <div class="form-group">
                    ...
                    <select name="device_id" id="deviceId" class="form-control">
                        <option value="">-- <?= __('none') ?> --</option>
                        <?php foreach ($availableDevices as $dev): ?>
                            <option value="<?= $dev['id'] ?>">
                                <?= e($dev['brand_name']) ?> <?= e($dev['product_name']) ?> (#<?= $dev['id'] ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="closeModal('accessoryModal')">
                    <?= __('cancel') ?>
                </button>
...
<script>
    function openAddModal() {
        document.getElementById('modalTitle').textContent = '<?= __('add_accessory') ?>';
        document.getElementById('saveButton').textContent = '<?= __('save') ?>';
        document.getElementById('accId').value = '';
        document.getElementById('accessoryForm').reset();
        document.getElementById('repairCommentGroup').style.display = 'none';
        openModal('accessoryModal');
    }

    function openEditModal(acc) {
        document.getElementById('modalTitle').textContent = '<?= __('edit') ?>';
        document.getElementById('saveButton').textContent = '<?= __('update') ?>';
        
        document.getElementById('accId').value = acc.id;
        document.getElementById('accessoryType').value = acc.type_id;
        document.getElementById('purchaseDate').value = acc.purchase_date;
        document.getElementById('accName').value = acc.name;
        document.getElementById('accQuantity').value = acc.quantity;
        document.getElementById('purchasePrice').value = acc.purchase_price;
        document.getElementById('purchaseCurrency').value = acc.purchase_currency;
        document.getElementById('sellingPrice').value = acc.selling_price || '';
        document.getElementById('repairComment').value = acc.repair_comment || '';
        document.getElementById('deviceId').value = acc.device_id || '';
        
        // Show/hide repair comment
        if (acc.type_id == '6') {
            document.getElementById('repairCommentGroup').style.display = 'block';
        } else {
            document.getElementById('repairCommentGroup').style.display = 'none';
        }
